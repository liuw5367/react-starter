...
function processResponseData(request: XMLHttpRequest, data: ApiResponseType) {
  ...
  const path = domainAndPath.slice(index) || ''

  // // 是否成功，boolean 类型 方便页面判断
  // if (path.startsWith('/api/xx/')) {
...
export function request<R = any>(url: string, config?: AxiosRequestConfig): Promise<ApiResponseType<R>> {
  if (config)
    config.url = url
